...
def button_SSE():
	...
	if len(path) > 0:
		figure_SSE(path)
from figure_clolor import Figure_colors
def button_figure_color():
	path = filedialog.askopenfilename()
	n_clusters=int(e.get())
	if len(path)>0:
		Figure_colors(path=path,n_clusters=n_clusters)
...
e.pack()
btn2=Button(root,text="figure of color",command = button_figure_color)
